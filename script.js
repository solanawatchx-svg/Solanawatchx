...
for (const token of tokens) {
  ...
  if (virtualToken > 0 && virtualSol > 0) {
    solPerToken = virtualSol / virtualToken;
  }

  // liquidity in SOL
  const liqSol = tokensFromDev * solPerToken;

  // convert to USD (avoid negative / invalid)
  const liqUsd = Math.abs(liqSol * solUsd);

  token.liq = Number(liqUsd.toFixed(2)); // USD rounded
}
