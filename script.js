...
// --- calculate LiQ (SOL) using exact final formula ---
...
for (const token of tokens) {
  ...
  if (virtualToken > 0 && virtualSol > 0) {
    ...
    // no reserves available — deterministic fallback: 0 SOL
    solPerToken = 0;
  }

  const firstSwapSol = tokensFromDev * solPerToken;
  token.liq = Number(Math.abs(firstSwapSol).toFixed(6)); // store positive SOL
}
