...
async function fetchLiveTokens() {
    ...
    const virtualSol = parseFloat(token.virtualSolReserves || 0);
    const virtualToken = parseFloat(token.virtualTokenReserves || 1);
    token.liq = (virtualSol / virtualToken) * currentSolPrice; // our bonding curve formula
}
