document.addEventListener("DOMContentLoaded", () => {
    ...
    // =============================== 
        ...
        // ===============================
        // --- COMPUTE LIQUIDITY SOL ---
        // ===============================
        function computeLiquiditySol(token) {
            let liquidity_sol = 0;
            let dev_held = 0;
        ...
        async function fetchLiveTokens() { 
            try { 
                ...
                if (supabaseClient && newTokens.length > 0) {
                    for (const token of newTokens) {
                        const liquidity_sol = computeLiquiditySol(token);
                        const tokenData = {
                        ...
                        try {
                            const { error } = await supabaseClient
                                .from('tokens')
                                .upsert(tokenData, { onConflict: 'coinMint' });
                            if (error) throw error;
                        } catch (error) {
                            console.error('Error inserting token to Supabase:', error.message);
                        }
                    }
                }
            ...
            function createTokenElement(token) { 
                let liquidity_sol = computeLiquiditySol(token);
